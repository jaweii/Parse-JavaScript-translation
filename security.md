...
介于CLP、ACL和指针权限间的复杂影响，我们建议你同时使用时要小心对待。使用CLP禁用某个类型的请求，然后其他请求使用指针权限或者ACL，这经常被用到。举个例子，你可能想禁用Photo表中的Delete权限，但另一方面又增加一个指针权限以便创建者可以编辑它，只是不允许删除它。因为指针权限和ACL的相互作用尤其复杂，我们一般推荐你只是用这两种安全机制中的一种。
